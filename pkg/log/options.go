...
// log 包的 Options 结构体支持3中方法：
// Build ：可以根据 Options 构建一个全局的 Logger
// AddFlags ：可以讲 Options 的各个字段追加到传入的 pflag.FlagSet 变量中
// String：将 Options 的值以 JSON 格式的字符串返回
type Options struct {
	// 支持输出到多个输出，用逗号隔开。支持输出到标准输出(stdout)和文件。
	OutputPaths []string `json:"output-paths"       mapstructure:"output-paths"`
	// zap 内部（非业务） 错误日志输出路径，若为多个输出，用逗号分开
	ErrorOutputPaths []string `json:"error-output-paths" mapstructure:"error-output-paths"`
	Level            string   `json:"level"              mapstructure:"level"`
	// 支持的日志输出格式，目前支持 Console 和 JSON 两种。Console 是Text 格式
	Format string `json:"format"             mapstructure:"format"`
	// 是否开启 caller， 如果开启，则会在日志中显示调用日志所在的文件、函数和行号
	DisableCaller bool `json:"disable-caller"     mapstructure:"disable-caller"`
	// 是否在 Panic 及以上级别禁止打印堆栈信息
	DisableStacktrace bool `json:"disable-stacktrace" mapstructure:"disable-stacktrace"`
	// 是否开启颜色输出。true 表示开启，false：禁止
	EnableColor bool `json:"enable-color"       mapstructure:"enable-color"`
	// 是否是开发模式，如果是开发模式，则会对 DPanicLevel 进行堆栈跟踪
