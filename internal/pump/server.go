...
import (
	...
	"github.com/nico612/iam-demo/internal/pump/pumps"
	"github.com/nico612/iam-demo/internal/pump/storage"
	"github.com/nico612/iam-demo/internal/pump/storage/redis"
...
func createPumpServer(cfg *config.Config) (*pumpServer, error) {
	...
	rs := redsync.New(goredis.NewPool(client)) // 创建一个 redsync 实例用于获取互斥锁

	server := &pumpServer{
		secInterval:    cfg.PurgeDelay,
		omitDetails:    cfg.OmitDetailedRecording,
		mutex:          rs.NewMutex("iam-pump", redsync.WithExpiry(10*time.Minute)), // 分布式锁实例
